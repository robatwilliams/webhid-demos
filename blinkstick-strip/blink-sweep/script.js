import { OFF, ORANGE } from '../colors.js';
...
async function handleStart() {
  ...
  const effectGenerator = blinkSweep();
...
function* blinkSweep() {
  // https://www.youtube.com/watch?v=_mqiQ3vILD8
  const SWEEP_DURATION = 250;
  const OFF_DURATION = 700;

  while (true) {
    for (let i = 0; i < LED_COUNT; i++) {
      yield [i, ORANGE];
      yield SWEEP_DURATION / LED_COUNT;
    }
